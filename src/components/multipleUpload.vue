...
.img_upload .uploader-icon .model {
  position: absolute;
  z-index: 999999;
  width: 100px;
  height: 100px;
...
.img_upload .uploader-icon .model span,
a {
  flex: 1;
  display: flex;
...
.img_upload .uploader-icon .model span:hover,
a:hover {
  color: #f69110;
}
...
.input_image:hover + .icon-updater {
  border-color: #409eff;
}
</style>
<template>
    <div class="img_upload" :v-loading.body="loading">
        <form>
            <input ref="imageUpload" type="file" @change="previewImg" :class="{'input_imgUrL': imgUrl}" class="input_image" name="photo" accept=".ico,image/gif,image/jpeg,image/jpg,image/png,image/svg0">
            <img src="../assets/images/icon_add.png" size="small" class=" icon-updater" :loading="loading">
            </img>
            <div class="uploader-icon" style="display:inline-block;" v-show="imgUrl" v-for="(item,index) in imgUrl" v-on:mouseenter="showModel = true" v-on:mouseleave="showModel = false">
                <img  v-bind:src="item" class="icon">
                <div class="model" v-show="showModel">
                    <a  :href="imgUrl" target="_blank" class="el-icon-view"></a>
                    <span class="el-icon-delete2" @click="deleteImgUrl(index)"></span>
  ...
  methods: {
      ...
      function newBlob(data, datatype) {
        ...
        try {
          ...
          if (e.name == "TypeError" && window.BlobBuilder) {
            ...
            throw new Error(
              "Your browser does not support Blob & BlobBuilder!"
            );
          }
        }
        return out;
    ...
    upload(file) {
      ...
      let body = {
        ...
        sign: "",
        biz_param: {
          bucketName: _self.iconParam.keyName
        }
      };
      ...
      httpService.commonPost(
        ...
        function(res) {
          if (res.code == "1c01") {
            ...
            // console.log("343343489999999999999999",res.biz_result.token);
...
　　　　　handler(curVal,oldVal){
          console.log('啥都看不见阿斯利康打不开链接撒',curVal,oldVal)
　　　　　},
　　　　　　deep:true
　　　　　}
　　},
