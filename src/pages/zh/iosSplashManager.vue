...
<template>
	<div class="content">
		...
		<el-dialog title="banner" v-model="dialogShow.diaglogMediaDetail">
			<addIOSSplash :splashParam="splashDetail" v-on:addSplash="recieveAdd">
			</addIOSSplash>
		</el-dialog>
	</div>
...
<script>
	...
	import addIOSSplash from '../../components/addIOSSplash.vue'
	let param = {
		pn: 1,
		pSize: 10,
		type: "1", // 0/1 android/ios
		cmsType: 1, //cms专用
	};
	...
	function fetchItem(store, val) {
		...
		return store.dispatch('getIosSplashList', {
			body: body,
			path: url
		});
	}
	export default {
		...
		computed: {
			...
			splashData() {
				return this.$store.state.splash.iosSplashList.list;
			},
			total() {
				return this.$store.state.splash.iosSplashList.list.length;
			}
		},
		created() {
			if(this.$store.state.splash.iosSplashList.list.length == 0) {
				this.getHttp();
			}
		},
		methods: {
			...
			openSplash(index, status) {
				...
				this.$store.dispatch('getIosSplashList', {
					body: body,
					path: url
				}).then(() => {
					_self.$message({
