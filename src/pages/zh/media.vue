	...
	<div class="content">
		...
		<div class="table">
            ...
            width:1329px;margin:auto" max-height="600" v-loading.body="loading">
				...
				<el-table-column label="新闻时间" width="150">
					<template scope="scope">
...
<script>
	...
	function fetchItem(store, val) {
		...
		return store.dispatch('getMediaList', {
			body: body,
			path: url
		});
	}
	...
	export default {
		...
		computed: {
			...
			mediaList() {
				return this.$store.state.media.mediaList.list
			},
			total() {
		...
		methods: {
			...
			edit(index) {
				...
				this.mediaDetail.newsTime = this.mediaList[index].newsTime;
