...
//图标文件类型
export function iconFileType(type){
    switch(type){
        case "0":
            return  '底部导航ICON';
        case "1":
...
//图标文件类型建议
export function iconFileTypeRecommend(queryString){
    return (typeRecommendList) => {
        return (typeRecommendList.value.indexOf(queryString.toLowerCase()) === 0);
      };
}
